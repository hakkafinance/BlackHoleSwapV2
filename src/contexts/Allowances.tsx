...
type AllowancesContext = any[]

type AllowancesState = {}

const UPDATE = 'UPDATE'

const AllowancesContext = createContext<AllowancesContext>([])

function useAllowancesContext() {
  return useContext(AllowancesContext)
}

function reducer(state: AllowancesState, { type, payload }: { type: 'UPDATE', payload: any }): AllowancesState {
  switch (type) {
    case UPDATE: {
      const {
        chainId,
...
export function useAddressAllowance(tokenAddress: string, address: string, spenderAddress: string) {
  ...
  const { value, blockNumber } =
    safeAccess(state, [chainId as number, address, tokenAddress, spenderAddress]) || {}
