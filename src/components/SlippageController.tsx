...
type SlippageControllerProps = {
  onChange: (value: number) => void
}

export default function SlippageController(
  props: SlippageControllerProps,
): JSX.Element {
  ...
  const updateSlippage = useCallback(
    (newSlippage: string) => {
      ...
      const numParsed = parseFloat(newSlippage) * 100
      onChange(numParsed)
    },
    [onChange],
  )
  ...
  const parseInput = (e: React.FormEvent<HTMLInputElement>) => {
