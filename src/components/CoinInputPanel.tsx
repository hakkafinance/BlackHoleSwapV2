...
import CoinSelector, { SelectorOption } from '../components/CoinSelector'
...
type CoinInputPanelProps = {
  title: string
  coinOptions?: SelectorOption[]
  selectedCoin: string
  value: string
  showUnlock?: boolean
  showMax?: boolean
  ...
  renderCoinButtons?: () => JSX.Element
  renderInputMessage?: () => JSX.Element
}

export default function CoinInputPanel(props: CoinInputPanelProps): JSX.Element {
  const {
    title,
    coinOptions,
    selectedCoin,
  ...
  const balance = useAddressBalance(selectedCoin, account as string)
  ...
  const _renderCoinButtons = () => {
    ...
    if (coinOptions && coinOptions.length) {
      return (
        <Row>
          {showUnlock && account && (
      ...
      <SpaceBetweenRow>
        ...
        {showMax && (
          <TextButton
            onClick={() => {
              if (balance) {
                onValueChange(formatUnits(balance, decimals))
              }
            }}
          >
            MAX
